...
export const investigateCostSpikeSkill: SkillDefinition = {
  ...
  description:
    'Investigate why AWS costs increased - identifies which services drove the cost increase',
  version: '1.0.0',
  ...
  steps: [
    {
      ...
      parameters: {
        command:
          'aws ce get-cost-and-usage --time-period Start={{period_start}},End={{period_end}} --granularity MONTHLY --metrics "BlendedCost" --group-by Type=DIMENSION,Key=SERVICE --region us-east-1',
    ...
    },
    {
      ...
      parameters: {
        command:
          'aws ce get-cost-and-usage --time-period Start={{compare_period_start}},End={{compare_period_end}} --granularity MONTHLY --metrics "BlendedCost" --group-by Type=DIMENSION,Key=SERVICE --region us-east-1',
...
Format as a table showing: Service | Previous Cost | Current Cost | Increase | % Change`,
    ...
    },
    {
      ...
      parameters: {
        query:
          'List all resources for the service identified as the top cost driver in {{steps.analyze_cost_changes.result}}',
      },
      onError: 'continue',
    },
    {
      ...
      parameters: {
        command:
          'aws cloudtrail lookup-events --lookup-attributes AttributeKey=EventName,AttributeValue=Create --start-time {{period_start}} --end-time {{period_end}} --max-items 50 --region us-east-1',
        reason:
          'Check for resources created during the spike period that might explain the cost increase',
      },
      onError: 'continue',
    },
    {
